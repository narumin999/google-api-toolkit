...
export async function getDatabase(htmlFolderId, dbFileName = 'records.json') {
    const q = `name='${dbFileName}' and '${htmlFolderId}' in parents and trashed=false`;
...
export async function saveDatabase(htmlFolderId, existingFileId, dbData, dbFileName = 'records.json') {
    const boundary = '-------314159265358979323846';
    const delimiter = "\r\n--" + boundary + "\r\n";
    const close_delim = "\r\n--" + boundary + "--";
    const metadata = { name: dbFileName, mimeType: 'application/json' };
...
export async function uploadFileToDrive(base64Data, filename, mimeType, folderId) {
    ...
    const multipartRequestBody = delimiter + 'Content-Type: application/json\r\n\r\n' + JSON.stringify(metadata) + delimiter + 'Content-Type: ' + safeMimeType + '\r\nContent-Transfer-Encoding: base64\r\n\r\n' + base64 + close_delim;
    const res = await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id', { method: 'POST', headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': `multipart/related; boundary=${boundary}` }, body: multipartRequestBody });
    const data = await res.json();
    return data.id; 
}

export async function saveHtmlFile(htmlFolderId, existingFileId, title, displayDate, locationStr, text, uploadedFiles) {
    let locationHtml = '';
    if (locationStr) {
